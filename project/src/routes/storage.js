...
router.post('/store', async (req, res) => {
    const { primaryKey, secondaryKey, value } = req.body;
    try {
        const tx = await storageContract.store(primaryKey, secondaryKey, value);
        const receipt = await tx.wait();
        res.json({
            message: `Dato almacenado con éxito para las claves: ${primaryKey} -> ${secondaryKey} -> ${value}`,
...
router.get('/retrieve', async (req, res) => {
    ...
    try {
        const [value, isSet] = await storageContract.retrieve(primaryKey, secondaryKey);
        res.json({
            primaryKey,
            secondaryKey,
            value,
