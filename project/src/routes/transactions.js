...
router.get('/:transactionHash', async (req, res) => {
    ...
    if (!transactionHash) {
        return res.status(400).json({ error: 'El parámetro transactionHash es obligatorio.' });
    }
    ...
    try {
        ...
        console.error(`Error al obtener el estado de la transacción: ${error.message}`);
        return res.status(500).json({ error: 'Error al consultar el estado de la transacción.' });
    }
});
