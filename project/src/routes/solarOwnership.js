...
router.post('/assign', authenticate, async (req, res) => {
    ...
    try {
        const tx = await ownershipContract.assignOwnership(req.authentication.address, percentage);
...
router.post('/transfer', authenticate, async (req, res) => {
    ...
    try {
        const tx = await ownershipContract.transferOwnership(toAddress, percentage, { from: address });
        const receipt = await tx.wait(); // Esperar confirmación
        res.json({
...
router.post('/transferByOwner', authenticate, async (req, res) => {
    ...
    try {
        ...
        const tx = await ownershipContract.transferOwnershipByOwner(fromAddress, toAddress, percentage);
        const receipt = await tx.wait(); // Esperar confirmación
        res.json({
...
router.get('/available', async (req, res) => {
    try {
        const availablePercentage = await ownershipContract.getAvailablePercentage();
